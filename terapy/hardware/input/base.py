...
class InputWidget(wx.ScrolledWindow):
	...
	def __init__(self,parent=None, units=["V"], qtynames=["X"], title="", input = None, min = None, max = None):
		...
		self.timer = InputWidgetUpdateThread(self)
	...
	def RefreshDisplay(self, val):
		"""
		
			Refresh display.
			
			Parameters:
				val	-	values to display (list of float)
...
class AntennaWidget(wx.ScrolledWindow):
	...
	def __init__(self,parent=None, units="V", title="", amp = "0", device = None, min = None, max = None):
		...
		self.min = min
		self.max = max
		self.timer = AntennaWidgetUpdateThread(self)
...
class InputWidgetUpdateThread(WidgetUpdateThread):
	def __init__(self,widget):
		WidgetUpdateThread.__init__(self,widget)
	
	def read(self):
		try:
			val = self.widget.input.read()
		except:
			val = [0]
...
class AntennaWidgetUpdateThread(WidgetUpdateThread):
	def __init__(self,widget):
		WidgetUpdateThread.__init__(self,widget)
	
	def read(self):
		try:
			amp = self.widget.device.get_amplitude()
		except:
			amp = 0
